...
@section('content')
<div class="mb-6">
    <h1 class="text-3xl font-bold text-gray-900 dark:text-gray-100">Edit Navigation Item</h1>
</div>
    ...
    <form action="{{ route('admin.navigation.update', $navigation) }}" method="POST">
        ...
        <div class="mb-4">
            ...
            <input type="text" 
                   name="route" 
                   id="route" 
                   value="{{ old('route', $navigation->route) }}"
        ...
        <div class="mb-4">
            ...
            <select name="section" 
                ...
                @foreach($sections as $key => $section)
                    <option value="{{ $key }}" {{ old('section', $navigation->section) == $key ? 'selected' : '' }}>
                        {{ $section['label'] }}
                    </option>
                @endforeach
            </select>
        ...
        <div class="mb-4">
            ...
            <select name="icon" 
                ...
                @foreach($icons as $icon)
                    <option value="{{ $icon }}" {{ old('icon', $navigation->icon) == $icon ? 'selected' : '' }}>
                        {{ ucfirst($icon) }}
                    </option>
                @endforeach
            </select>
