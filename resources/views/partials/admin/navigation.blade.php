<nav class="mt-5 px-2">
    ...
    @foreach($navigationSections as $section)
        @if($navigationBySection->has($section['id']))
            <div class="mt-6 first:mt-0">
                <h3 class="px-3 text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider">
                    {{ $section['label'] }}
                </h3>
                <div class="mt-2 space-y-1">
                    @foreach($navigationBySection[$section['id']] as $item)
                    <a href="{{ isset($item['route_params']) ? route($item['route'], $item['route_params']) : route($item['route']) }}"
                        class="group flex items-center px-2 py-2 text-base leading-6 font-medium rounded-md text-gray-900 dark:text-gray-100 hover:bg-gray-200 dark:hover:bg-gray-700 transition">
                        <svg class="mr-3 h-6 w-6 text-gray-500 group-hover:text-gray-900 dark:text-gray-400 dark:group-hover:text-gray-300 transition"
                            fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                d="{{ $iconPaths[$item['icon']] ?? $iconPaths['document'] }}" />
                        </svg>
                        {{ $item['name'] }}
                    </a>
                    @endforeach
